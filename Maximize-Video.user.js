...
exec(function() {
  ...
  if (navigator.language.toLocaleLowerCase() == 'zh-cn') {
    gv.btnText = {
      ...
      tip: 'Iframe Video, please click the video with your mouse and try again'
    };
  } else {
    gv.btnText = {
      max: 'Maximize',
  ...
  var tool = {
    ...
    addTip: async function(str) {
      if (!document.getElementById('catTip')) {
        ...
        tip.id = 'catTip';
        tip.innerHTML = str;
        tip.style.cssText = 'transition: all 0.8s ease-out;background: none repeat scroll 0 0 #27a9d8;color: #FFFFFF;font: 1.1em "Fira Code";margin-left: -250px;overflow: hidden;padding: 10px;position: fixed;text-align: center;bottom: 100px;z-index: 300;',
  ...
  var handle = {
    ...
    hotKey: function(e) {
      ...
      //The default exit key is ESC。Search for other shortcuts "keycode"，Modify to the number corresponding to the key。
      if (e.keyCode == 27) {
        maximize.playerControl();
      }
      //The default picture-in-picture shortcut is F2。
      if (e.keyCode == 113) {
        pictureInPicture();
      }
    },
